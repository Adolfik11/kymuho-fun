...
BOT_TOKEN = os.getenv("BOT_TOKEN") or globals().get("BOT_TOKEN", None)
...
# === SAFE SENDING HELPERS (auto-inserted) ===
async def safe_send(bot, chat_id, text, parse_mode=None, **kwargs):
    # Safe wrapper for bot.send_message:
    # - if parse_mode contains 'Markdown', escape text and use MarkdownV2
    try:
        if parse_mode and 'Markdown' in str(parse_mode):
            text = escape_md(text)
...
async def safe_reply(update, text, parse_mode=None, **kwargs):
    # Safe wrapper for update.message.reply_text
    try:
        if parse_mode and 'Markdown' in str(parse_mode):
            text = escape_md(text)
            parse_mode = 'MarkdownV2'
        ...
        pass
    return await safe_reply(update,text, parse_mode=parse_mode, **kwargs)

async def safe_query_reply(query, text, parse_mode=None, **kwargs):
    # Safe wrapper for query.message.reply_text and query.message.edit_text usages.
    try:
        if parse_mode and 'Markdown' in str(parse_mode):
            text = escape_md(text)
            parse_mode = 'MarkdownV2'
        ...
        pass
    try:
        return await safe_query_reply(query,text, parse_mode=parse_mode, **kwargs)
    except Exception:
        try:
            return await query.message.edit_text(text, parse_mode=parse_mode, **kwargs)
        except Exception:
            return await query.answer()
# === end safe sending helpers ===

# Safe removal of character from DB (uses existing get_db_connection if available)
def remove_character_from_collection(user_id, character_name):
    try:
        ...
        pass
    # fallback: try user_collections table if present
    try:
        if 'get_db_connection' in globals():
            conn = get_db_connection()
            c = conn.cursor()
            c.execute("DELETE FROM user_collections WHERE user_id=? AND character_name=?", (user_id, character_name))
            conn.commit()
            conn.close()
            return True
    except Exception:
...
# Simple health server helper (only start if not already defined)
try:
    start_health_check_server  # type: ignore
except NameError:
...
# Autoping task to keep instance alive (for Render)
async def _health_autoping(interval_sec: int = 300):
    url = f'http://127.0.0.1:{int(os.environ.get("PORT", 10000))}/health'
    while True:
        try:
            def _ping():
                try:
...
# Global error sender to admin (best-effort)
async def _send_error_to_admin(application, text):
    try:
...
async def show_subscription_required(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message:
        ...
        await safe_query_reply(query,text, reply_markup=reply_markup)
...
def init_db():
    ...
    try:
        ...
        conn.commit()
        logger.info("Database initialized successfully with corrected user table")
    except sqlite3.Error as e:
...
def update_user_balance(user_id, amount):
    """Обновление баланса пользователя с проверкой - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
    conn = get_db_connection()
    try:
        c = conn.cursor()
        ...
        if not result:
            # Пользователь не существует - создаем с начальным балансом 100
            initial_balance = 100 + amount if amount > 0 else 100
            c.execute('INSERT INTO users (user_id, balance) VALUES (?, ?)', 
                     (user_id, initial_balance))
            conn.commit()
            return amount > 0  # Для положительных сумм - успех, для отрицательных - нет
...
def get_user_balance(user_id):
    """Получение баланса пользователя - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
    conn = get_db_connection()
    try:
        c = conn.cursor()
        c.execute('SELECT balance FROM users WHERE user_id = ?', (user_id,))
        ...
        if result:
            ...
            # Если пользователя нет, создаем с начальным балансом
            c.execute('INSERT INTO users (user_id, balance) VALUES (?, 100)', (user_id,))
            conn.commit()
            return 100
    except sqlite3.Error as e:
...
async def collection_command(query, context):
    ...
    for rarity, data in CHARACTER_RARITY.items():
        ...
        text += f"{data['emoji']} {rarity.capital()}: {count}\n"
...
# === ИСПРАВЛЕННЫЙ ОБРАБОТЧИК ПОКУПКИ ===
async def buy_item_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка покупки в магазине - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
    query = update.callback_query
    await query.answer()
    
    user = query.from_user
    
    # ПРАВИЛЬНОЕ извлечение ID товара из callback_data
    # callback_data имеет формат "buy_itemid", где itemid - идентификатор товара
    item_id = query.data.replace('buy_', '')  # Убираем префикс "buy_"
    
    if item_id not in SEASONAL_SHOP:
        await safe_edit_message(query, f"❌ Товар '{item_id}' не найден в магазине!")
        return
    ...
    if not success:
        await safe_edit_message(query, "❌ Ошибка при списании средств!")
        return
    ...
    if item['type'] == 'gacha':
        ...
        # Для других типов товаров можно добавить обработку
        await safe_edit_message(query, f"✅ Товар '{item['name']}' успешно куплен!")
...
async def start_pvp_battle(context, player1_id, player2_id):
    ...
    try:
        await safe_send(context.bot,chat_id=player1_id, text=battle_text)
        await safe_send(context.bot,chat_id=player2_id, text=battle_text)
    except Exception as e:
...
# === ENTRY POINT (replaced by assistant) ===
if __name__ == '__main__':
    try:
        # Ensure BOT_TOKEN is set
        if not globals().get('BOT_TOKEN'):
            # try to fetch from original if exists
            try:
                BOT_TOKEN = os.getenv('BOT_TOKEN') or globals().get('BOT_TOKEN') or None
            except Exception:
                BOT_TOKEN = None
        if not BOT_TOKEN:
            print("ERROR: BOT_TOKEN not set in environment or globals. Exiting.")
            raise SystemExit(1)
        # start health server if not already
        try:
            start_health_check_server()
        except Exception:
            pass
        # run autoping in background via asyncio
        try:
            import asyncio
            loop = asyncio.get_event_loop()
            loop.create_task(_health_autoping(300))
        except Exception:
            pass
        # Run original main if exists, else try to run an async main function
        try:
            # If original defined a main() function, call it
            if 'main' in globals() and callable(globals()['main']):
                # If main is async, use asyncio.run
                m = globals()['main']
                if asyncio.iscoroutinefunction(m):
                    asyncio.run(m())
                else:
                    # run in thread to keep compatibility
                    m()
            else:
                # fallback: do nothing
                print("No main() found; bot module merged.")
        except Exception as e:
            import traceback
            tb = traceback.format_exc()
            print("Fatal error in main:", e)
            print(tb)
            # try to notify admin via Telegram (best-effort)
            try:
                import telegram
                from telegram import Bot
                bot = Bot(token=BOT_TOKEN)
                bot.send_message(chat_id=list(ADMIN_IDS)[0], text=f"Fatal error starting bot: {escape_md(str(e))}\n{escape_md(tb[:4000])}")
            except Exception:
                pass
    except SystemExit:
        raise
...
# === ДОПОЛНИТЕЛЬНЫЙ ЛОГ И КОМАНДА /ping ===
from telegram.ext import CommandHandler

async def ping(update, context):
    user = update.effective_user
    message = update.effective_message.text if update.effective_message else "нет текста"
    logging.info(f"📩 Получен апдейт от {user.id if user else 'unknown'}: {message}")
    await safe_reply(update, "pong 🟢", parse_mode='MarkdownV2')
...
async def main():
    ...
    logging.info("✅ Запуск health-сервера...")
    try:
        start_health_check_server()
    except Exception as e:
        logging.warning(f"Health server не запущен: {e}")

    logging.info("🤖 Запуск Telegram-приложения...")
    app = ApplicationBuilder().token(BOT_TOKEN).build()

    # === РЕГИСТРАЦИЯ КОМАНД ===
    app.add_handler(CommandHandler("start", start))
    app.add_handler(CommandHandler("help", help_command if 'help_command' in globals() else start))
    if 'profile_command' in globals():
        app.add_handler(CommandHandler("profile", profile_command))
    if 'admin_command' in globals():
        app.add_handler(CommandHandler("admin", admin_command))
    if 'trade_command' in globals():
        app.add_handler(CommandHandler("trade", trade_command))
    if 'give_command' in globals():
        app.add_handler(CommandHandler("give", give_command))
    if 'broadcast_command' in globals():
        app.add_handler(CommandHandler("broadcast", broadcast_command))
    if 'clear_pvp_command' in globals():
        app.add_handler(CommandHandler("clear_pvp", clear_pvp_command))

    # === CALLBACK-ХЭНДЛЕРЫ ===
    app.add_handler(CallbackQueryHandler(menu_handler))
    if 'check_subscription_handler' in globals():
        app.add_handler(CallbackQueryHandler(check_subscription_handler, pattern="check_subscription"))
    if 'buy_item_handler' in globals():
        app.add_handler(CallbackQueryHandler(buy_item_handler, pattern=r"^buy_"))

    logging.info("🎯 Все обработчики успешно зарегистрированы.")
    app.add_handler(CommandHandler('ping', ping))
    logging.info('✅ Команда /ping успешно зарегистрирована.')

    # === СТАРТ БОТА ===
    logging.info("🤖 Бот готов к работе! Ожидание обновлений...")
...
if __name__ == "__main__":
    try:
        ...
        import traceback
        logging.error(f"Фатальная ошибка запуска: {e}\n{traceback.format_exc()}")
